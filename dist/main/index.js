...
const AjvModule = /* @__PURE__ */ getDefaultExportFromCjs(ajvExports$1);
var dist = { exports: {} };
var formats = {};
(function(exports) {
  Object.defineProperty(exports, "__esModule", { value: true });
...
if (customUserData) {
  try {
    electron.app.setPath("userData", customUserData);
  } catch {
  }
}
...
async function createSplash() {
  const showSplash = electron.app.isPackaged && process.env.CRIMSON_NO_SPLASH !== "1";
  ...
  try {
    const tmpPath = path.join(electron.app.getPath("userData"), "splash.html");
    await fs$1.writeFile(tmpPath, splashHtml, "utf-8");
...
async function createWindow() {
  ...
  mainWindow.webContents.on("did-fail-load", (_e, code2, desc) => {
    console.warn("Renderer failed to load:", code2, desc);
...
electron.ipcMain.handle("fs:saveText", async (_evt, { defaultPath, content }) => {
  const result = await electron.dialog.showSaveDialog({ defaultPath });
  if (result.canceled || !result.filePath) return null;
  await fs$1.writeFile(result.filePath, content, "utf-8");
  return result.filePath;
});
electron.ipcMain.handle("fs:openText", async () => {
  const result = await electron.dialog.showOpenDialog({
    properties: ["openFile"],
...
electron.ipcMain.handle("fs:exportMany", async (_evt, files) => {
  ...
  await Promise.all(files.map((f) => {
    ...
    if (f.encoding === "base64") {
      ...
      return fs$1.writeFile(filePath, buf);
    }
    return fs$1.writeFile(filePath, f.content, "utf-8");
  }));
  return dir;
});
electron.ipcMain.handle("store:get", (_evt, key) => {
